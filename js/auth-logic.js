...
// ── ✅ FIX: Logout — index.html वर redirect (home/home.html नाही!) ──
window.logoutUser = () => {
  if (typeof toast === 'function') toast('Logging out...', '');
  signOut(auth).then(() => {
    window.location.href = "index.html"; // ✅ Root level index.html
  }).catch((error) => {
...
window._firebaseGoogleLogin = async function() {
  try {
    ...
    // ✅ FIX: Mobile आणि GitHub Pages दोन्हींसाठी Redirect वापरतो
    // Popup बऱ्याचदा mobile वर block होतो
    const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);

    if (isMobile) {
      toast('Redirecting to Google...', '');
      await signInWithRedirect(auth, provider);
      // redirect नंतर page reload होतो, getRedirectResult खाली handle करतो
    } else {
      // Desktop: Popup वापरतो (faster UX)
    ...
    if (err.code === 'auth/popup-closed-by-user') {
      ...
      // Popup block झाला → redirect वर fallback
...
// ── ✅ FIX: Redirect Result Handle ──
// Google Redirect login नंतर page reload होतो.
// getRedirectResult() ने result मिळवतो आणि home.html वर जातो.
try {
  ...
  if (result && result.user) {
    ...
    toast('✓ ' + name + ' — Google Login Successful!', 'success');
    setTimeout(() => showSuccess(name), 200);
    // showSuccess → enterGame → home/home.html वर redirect
  }
} catch (err) {
